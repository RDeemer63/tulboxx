...
export default function Estimates() {
  ...
  const getStatusColor = (status: string) => {
    const colors: { [key: string]: string } = {
      draft: "bg-gray-100 text-gray-800 dark:bg-slate-800/50 dark:text-slate-300",
      ...
      accepted: "bg-green-100 text-green-800 dark:bg-green-800/20 dark:text-green-300",
      rejected: "bg-red-100 text-red-800 dark:bg-red-800/20 dark:text-red-300",
      expired: "bg-yellow-100 text-yellow-800 dark:bg-yellow-800/20 dark:text-yellow-300"
    };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <Card className="bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-700">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
            <CardTitle className="text-sm font-medium text-gray-900 dark:text-white">Total Estimates</CardTitle>
            <FileText className="h-4 w-4 text-gray-500 dark:text-slate-400" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-gray-900 dark:text-white">{estimates.length}</div>
        ...
        <Card className="bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-700">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
            <CardTitle className="text-sm font-medium text-gray-900 dark:text-white">Total Pipeline Value</CardTitle>
            <DollarSign className="h-4 w-4 text-gray-500 dark:text-slate-400" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold text-gray-900 dark:text-white">{formatCurrency(totalEstimateValue)}</div>
            <p className="text-xs text-gray-500 dark:text-slate-400">
      ...
      <Card className="bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-700">
        <CardHeader>
          <CardTitle className="text-gray-900 dark:text-white">Estimates ({filteredEstimates.length})</CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
              ...
              <TableBody>
                {filteredEstimates.map((estimate) => (
                  <TableRow key={estimate.id} className="group hover:bg-slate-50 dark:hover:bg-slate-800/50">
                    ...
                    <TableCell>
                      <div>
                        <p className="font-medium text-slate-900 dark:text-slate-100">
                          {estimate.customer?.firstName || 'N/A'} {estimate.customer?.lastName || ''}
                        </p>
                    ...
                    <TableCell>
                      <Badge className={`${getStatusColor(estimate.status)} flex items-center space-x-1 w-fit`}>
