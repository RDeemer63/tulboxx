...
export const LoadingSpinner: React.FC<LoadingSpinnerProps> = ({
  ...
  const spinnerClasses = `animate-spin text-primary ${sizeClasses[size]} ${className}`;
  ...
  if (variant === "fullPage") {
    return (
      <div className="fixed inset-0 bg-slate-900/20 backdrop-blur-sm z-50 flex flex-col items-center justify-center">
        ...
        {text && <p className="text-primary mt-4 font-medium">{text}</p>}
      </div>
    );
  }

  // Overlay on parent element
  if (variant === "overlay") {
    return (
      <div className="absolute inset-0 bg-white/80 dark:bg-slate-900/80 z-10 flex flex-col items-center justify-center">
        <svg className={spinnerClasses} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
          <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        {text && <p className="text-primary mt-2 text-sm font-medium">{text}</p>}
      </div>
    );
  }
